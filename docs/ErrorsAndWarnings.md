...
| Error code | Message | Description |
| ---------- | ------- | ----------- |
| BVE1400 | Language '{0}' is not supported. | Something went wrong with the module. Please open an issue. |
| BVE1401 | The 'OutputPath' parameter is missing or empty. | Something went wrong with the module. Please open an issue. |
| BVE1402 | The file '...' could not be created. | There was an error writing the source file with the  specified assembly attributes. |
| BVE1403 | The parameter '...' has an invalid name. | A `LiteralAssemblyAttribute` item's metadata has a name starting with `_Parameter`, but what follows could not be parsed as an integer number. |
| BVE1404 | The parameter '...' has an invalid parameter index. | A `LiteralAssemblyAttribute` item's metadata has a name starting with `_Parameter` followed by an integer number lower than 1 (e.g. `_Parameter0` or `_Parameter-5`). |
| BVE1405 | The parameter '...' was supplied, but not all previously numbered parameters. | A `LiteralAssemblyAttribute` item's metadata specifying one or more numbered parameters are not in a 1-based sequence (e.g. there is a `_Parameter2` but no `_Parameter1`, or there are `_Parameter1`  and `_Parameter3` but no `_Parameter2`). |
| BVE1406 | The parameter '...' is empty. | A `LiteralAssemblyAttribute` item's metadata has empty content. To specify the empty string as a parameter, use `""` (e.g. `<_Parameter1>""<_Parameter1>`).
...
## ThisAssemblyClass module (1800-1899)

| Error code | Message | Description |
| ---------- | ------- | ----------- |
| BVE1800 | Language '...' is not supported. | Something went wrong with the module. Please open an issue. |
| BVE1801 | The OutputPath parameter is missing or empty. | Something went wrong with the module. Please open an issue. |
| BVE1802 | The file "..." could not be created. | There was an error writing the source file with the generated class. |
| BVE1803 | Constant '...' has no '...' metadata. | The specified `ThisAssemblyConstant` item is missing required metadata. |
| BVE1804 | Constant '...' has unknown or unsupported type '...'. | The specified `ThisAssemblyConstant` item contains an invalid `Type` metadata. |
| BVE1805 | Constant '...' has invalid value '...'. | The specified `ThisAssemblyConstant` item contains a `Value` metadata that cannot be successfully parsed as the type indicated by the `Type` metadata of the same item. |
