...
public static class CodeGeneratorConfigurationUtility
{
    ...
    {
        foreach (var pair in AllowedLanguages.Where(pair => string.Equals(str, pair.Key, StringComparison.OrdinalIgnoreCase)))
        {
            language = pair.Value;
            return true;
        }
    ...
    public static bool TryParseConstantValue(string? str, out object? result)
    {
        ...
        return colonPos < 1
            ...
            : TryParseTypedConstantValue(str[..colonPos], str[(colonPos + 1)..], out result);
    }
